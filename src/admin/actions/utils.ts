...
import { GAMELIST_RECORDS_TABLE, MY_BGG_RECORDS_TABLE, USER_AUTH_RECORDS_TABLE } from './config';
...
export const createTables = async () => {
  ...
  await sql(`
    CREATE TABLE IF NOT EXISTS ${MY_BGG_RECORDS_TABLE} (
      "recordId" INTEGER PRIMARY KEY GENERATED ALWAYS AS IDENTITY,
      "created" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
      "lastUpdated" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
      "username" TEXT NOT NULL,
      "gameListCount" INTEGER NOT NULL
    );
  `);
};
